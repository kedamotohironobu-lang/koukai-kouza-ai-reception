(function (global) {
  ...
  const FAQ = [
    ...
    {topic:'メール・申込手続き', answer:'正式資料では電話の後、所属、氏名、連絡先、希望する回と演題、受講方法を k-open@hyogo-c.ed.jp にメール送信します。この試作版の仮受付登録はメール送信や受講確定ではありません。画面の「確認メールを作成」からメールアプリを開き、利用者本人が内容を確認して送信します。検証中は画面の検証用宛先へ送ります。',source:'R8公開講座 前半・後半／問合せ記録用紙'},
  ...
  const instruction=[
    ...
    'submit_confirmed_receptionがok:trueを返すまで受付番号を作ったり登録成功を宣言しない。仮受付は受講確定ではなくメール送信も行わない。失敗や結果不明なら画面で確認を促す。確認メールは画面のボタンで利用者のメールアプリを開き、利用者本人が送信します。メールアプリを開いても送信済みとは判断できません。受信確認と照合はまだ確認できないため、受付完了・受講確定とは言わないでください。送信先は画面の設定を優先し、検証中は検証用宛先です。',
  ...
  class NaturalReception {
    ...
    async submit(){
      ...
      this.pending=(async()=>{try{const result=await this.o.submit(checked);this.completed={ok:true,...result,note:'仮受付登録のみ完了。次に画面の確認メールを作成ボタンでメールアプリを開き、利用者本人が送信してください。メール送信・受信確認・受講確定はしていません。'};return this.completed;}catch(_){return {ok:false,error:'登録結果を確認できませんでした。登録済みの可能性があります。画面で同じ内容を再確認してください。'};}finally{this.pending=null;}})();
      return this.pending;
    }
